...
export class StreamsManager {
  ...
  async getDocuments(cursor) {
    ...
    return await this.client.query(
      Paginate(
        Documents(Collection(process.env.REACT_APP_FAUNA_COLLECTION)),
        options
      )
    );
  }
  ...
  openStreams(documentReferences) {
    ...
    documentReferences.forEach((ref) => {
      ...
      documentStream
        .on('start', (data, event) =>
          this.handleStart(streamAndData, data, event)
        )
        ...
        .on('history_rewrite', (data, event) =>
          this.handleHistoryRewrite(data, event)
        )
        .on('error', (data, event) =>
          this.handleError(streamAndData, data, event)
        );
