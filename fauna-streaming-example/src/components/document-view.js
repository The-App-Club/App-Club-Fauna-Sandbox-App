...
export class DocumentView extends React.Component {
    ...
    return (
      <div className="documents-view">
        <div className="title">
          ...
          Below is an overview of a page of products that changes live when you
          make a change in your Fauna database.
          <br />
          When you move to the next page, the streams will be closed and new
          streams will be opened for the new page.
          <br />
          The current page size is set to {this.streamsManager.PAGESIZE}. <br />
          <br /> Each box represents a document in your collection. Each bullet
          represents an incoming event, click on one of the bullets to see the
          diff.
        </div>
        <div className="documents-page">{this.renderDocuments()}</div>
        <div className="links">
          <a className="button" onClick={this.getPrevious}>
            previous
          </a>
          <div className="current-page"> Page {this.streamsManager.page}</div>
          <a className="button" onClick={this.getNext}>
            {' '}
            next
          </a>
        </div>
  ...
  renderDocumentLine(currentAndDiff) {
    ...
    return (
      <div key={'line-' + id} className="document-line">
  ...
  renderDocumentNameAndIcon(id, currentAndDiff) {
    return (
      <div className="document-icon-name" key={'icon-name' + id}>
        <div className="document-name" key={'name' + id}>
          {' '}
          {currentAndDiff.current.name}{' '}
        </div>
        <img
  ...
  renderDocumentVersions(id, currentAndDiff) {
    return (
      <div className="document-versions" key={'versions' + id}>
        {currentAndDiff.diffs.map((diffAndDoc, versionIndex) =>
          this.renderDocumentVersion(id, versionIndex, diffAndDoc)
        )}
      </div>
    );
  }
